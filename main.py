...
from functions.function_executor import call_function
...
def main():
    ...
    if not args:
        ...
        print('Example: python main.py "List files in the project"')
        sys.exit(1)
...
def generate_content(client, messages, verbose):
    ...
    for function_call_part in response.function_calls:
        ...
        if (
            ...
            raise RuntimeError(f"Fatal: function call failed for {function_call_part.name}")

        resp = function_call_result.parts[0].function_response.response
        if verbose:
            print(f"-> {resp}")

        # Append response back to messages for next round
        if "result" in resp and resp["result"] is not None:
            messages.append(types.Content(role="user", parts=[types.Part(text=resp["result"])]))
